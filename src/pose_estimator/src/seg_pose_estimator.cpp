...
SegmentorPoseEstimator::SegmentorPoseEstimator() : yaw(0), roll(0), pitch(0), posX(0), posY(0), posZ(0), aligned(0)
{
...
bool SegmentorPoseEstimator::start(pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud, pcl::PointCloud<pcl::PointXYZ>::Ptr aligned_model,
...
void SegmentorPoseEstimator::clearAll()
{
...
void SegmentorPoseEstimator::voxel_filter(pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud)
{
...
void SegmentorPoseEstimator::RANSAC_segmentor(pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud)
{
...
void SegmentorPoseEstimator::square_exclusion_segmentor(pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud)
{
    ...
    if(temp->points.size() > 0)
    {
        ...
        pass_seg.setInputCloud(cloud);
        pass_seg.setFilterFieldName("z");
        pass_seg.setFilterLimits(0.05, 1.5);
...
void SegmentorPoseEstimator::pose_estimate(pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud, pcl::PointCloud<pcl::PointXYZ>::Ptr &model,
        Eigen::Matrix4d &prealign_mat, Eigen::Matrix4d &alignment_mat)
{
...
bool SegmentorPoseEstimator::setModel(pcl::PointCloud<pcl::PointXYZ>::Ptr model)
{
    pipe_model = model;
    model_assigned = true;
}

void SegmentorPoseEstimator::getSegmented(pcl::PointCloud<pcl::PointXYZ>::Ptr &seg_cloud)
{
    seg_cloud = segmented_pipe;
}

void SegmentorPoseEstimator::getPose(std::vector<float> &input_vec)
{
    if(aligned)
    {
...
void SegmentorPoseEstimator::getFiltered(pcl::PointCloud<pcl::PointXYZ>::Ptr &filtered_cloud)
{
