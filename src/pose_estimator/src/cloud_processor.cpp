...
#include <pose_estimator/pipe_pose.h>
...
private:
    ...
    ros::NodeHandle nh;
    ros::Publisher pub_box, pub_pin;
    pose_estimator::pipe_pose box_msg, pin_msg;
...
private:
    ...
    void start()
    {
        pub_box = nh.advertise<pose_estimator::pipe_pose>("Box_pose", 2);
        pub_pin = nh.advertise<pose_estimator::pipe_pose>("Pin_pose", 2);
        running = true;
        save = false;
        ...
        reader.read("/home/erlendb/Blender/box_end7_m.ply", *aligned_boxEnd); //box_end7_m.ply
        reader.read("/home/erlendb/Blender/pin_end15_05.ply", *aligned_pin); //pin_end_013.ply
            ...
            pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud, const darknet_ros_msgs_eb::BoundingBoxes &det_inf, int index) const
    {
        ...
        if(true/*det_inf.bounding_boxes.size() == 0*/)
        {
            ...
            switch(event.getKeyCode())
            {
                ...
                case 'd':
                    ...
                    if(poseData.is_open())
                    {
                        ...
                        poseData.open("/home/erlendb/Programmering/Matlab/angled_quaternions22_05.txt");
    ...
    void poseEstimation(const pcl::PointCloud<pcl::PointXYZ>::Ptr &input_box_cloud, const pcl::PointCloud<pcl::PointXYZ>::Ptr &input_pin_cloud)
    {
        ...
        double theta_y;
        double theta_y1 = -asin(box_transformation_matrix(2,0));
        double theta_y2 = M_PI + asin(theta_y1);
        theta_y = theta_y1;
        ...
        box_msg.pipe_x = box_transformation_matrix(0,3); box_msg.pipe_theta_x = theta_x;
        box_msg.pipe_y = box_transformation_matrix(1,3); box_msg.pipe_theta_y = theta_y;
        box_msg.pipe_z = box_transformation_matrix(2,3); box_msg.pipe_theta_z = theta_z;


        pose_pin.erase(pose_pin.begin(), pose_pin.begin()+6);
        theta_y1 = -asin(pin_transformation_matrix(2,0));
        theta_y2 = M_PI + asin(theta_y1);
        theta_y = theta_y1;
        ...
        pin_msg.pipe_x = pin_transformation_matrix(0,3); pin_msg.pipe_theta_x = theta_x;
        pin_msg.pipe_y = pin_transformation_matrix(1,3); pin_msg.pipe_theta_y = theta_y;
        pin_msg.pipe_z = pin_transformation_matrix(2,3); pin_msg.pipe_theta_z = theta_z;

        pub_box.publish(box_msg);
        pub_pin.publish(pin_msg);
